...
public interface RecordLogRepository extends JpaRepository<EnhanceRecord, Long>, RecordLogQueryRepository {
    ...
    @Query("select er1 from EnhanceRecord er1 where er1.score = (select max(er2.score) from EnhanceRecord er2 where er2.item.id = :item_id) order by er1.id desc limit 1")
